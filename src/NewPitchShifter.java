 ...
 */
public class NewPitchShifter implements AudioProcessor{
	...
	public double pitchShiftRatio = 0;
	...
	public NewPitchShifter(double factor, double sampleRate, int size, int overlap){
	...
	public boolean process(AudioEvent audioEvent) {
		...
		for(int i = 0 ; i < size/2 ; i++){
			int index = (int)(i * this.pitchShiftRatio);
			if(index < size/2){
				newMagnitudes[index] += currentMagnitudes[i];
				newFrequencies[index] = (float) (currentFrequencies[i] * this.pitchShiftRatio);
			}
		}
...
//		System.out.println("Ratio: " + this.pitchShiftRatio);
//		
